...
public class MenuEditorial {
   ...
   private final  EditorialPersistencia ediPer;

   
    public MenuEditorial() {
        this.sc = new Scanner(System.in);      
        this.ediServ = new EditorialServicio();
        this.ediPer = new EditorialPersistencia();
    ...
    public void menuEditorial() throws Exception {
        ...
        do {           
            System.out.println("------------------------------------------------------------------------------");
            System.out.println(" 1- Crear/Ingresar Editorial");
            System.out.println(" 2- Modificar/Editar Editorial");
            System.out.println(" 3- Eliminar Editorial");
            System.out.println(" 4- Mostrar/Listar a todas las Editoriales");
            System.out.println(" 5- Buscar Editorial por nombre");
            System.out.println(" 6- Buscar Editorial por código");
            ...
            switch (opcion) {
                case 1:
                        limpiarPantalla();
                        ediServ.creareditorial(cargarCodigoEditorial(), cargarNombre(), cargarAlta());
                    break;
                case 2:
                           limpiarPantalla();
                        ediServ.modificareditorial(cargarNombre(), cargarNombre());
                    break;
                case 3:
                        limpiarPantalla();
                        ediServ.eliminareditorial(cargarNombre());
                    break;
                case 4:
                       limpiarPantalla();
                       ediPer.listarEditorials();
                    break;
                case 5:
                       limpiarPantalla();
                       ediPer.buscarPorNombre(cargarNombre());
                    break;
                case 6:
                        limpiarPantalla();
                        ediPer.listarEditoralCodigo(cargarCodigoEditorial());
                    break;
                case 7:                 
                        limpiarPantalla();
                        System.out.println("Salió del menú  de gestión de editorial correctamente...\n");
                       cargarMenu();                
                    break;
                default:
    ...
    public Integer cargarCodigoEditorial() throws Exception {        
        System.out.println("Ingrese el codigo de la Editorial");
        Integer codigo = sc.nextInt();
        try {
            if (validarCodigo(codigo)) {
                System.out.println("Código ingresado NO registrado con anteriormente, continue con el registro de  datos...");
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
            cargarCodigoEditorial();
        }
        return codigo;
    }
    public boolean validarCodigo(Integer codigo) throws Exception {
        ...
        try {
            List<Editorial> editoriales = ediPer.listarEditorials();
            for (Editorial e : editoriales) {
                if (Objects.equals(e.getCodEditorial(), codigo)) {
